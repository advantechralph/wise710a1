...
#check arg number
if [ $# -lt 1 ];then
    echo "Usage: ./mksd_recovery-linux.sh /dev/sdb [ubuntu18044]"
    exit
fi
...
if [ -f "../image/ubuntu18044.tar.gz" ] && [ x${ROOTFS_VERSION} != 'xubuntu18044' ]; then
	...
	echo "./mksd_recovery-linux.sh /dev/sdb ubuntu18044"
	exit 0
fi
...
# for emmc update usage
	...
	# copy rootfs
	if [ x${ROOTFS_VERSION} = 'xubuntu18044' ]; then
    	cp -arxf ../image/ubuntu18044.tar.gz mount_point0/mk_inand/image/
	else
