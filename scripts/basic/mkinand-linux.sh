...
#check arg number
if [ $# -lt 1 ];then
	echo "Usage: ./mkinand-linux.sh /dev/mmcblk0 [ubuntu18044]"
	exit
fi

if [ -f "../image/ubuntu18044.tar.gz" ] && [ x${ROOTFS_VERSION} != 'xubuntu18044' ]; then
    echo "Ubuntu image detected. Please run following cmd to burn SD:"
	echo "./mkinand-linux.sh /dev/mmcblk0 ubuntu18044"
	exit 0
fi
...
if [ x${ROOTFS_VERSION} = 'xubuntu18044' ]; then
	filesystem=../image/ubuntu18044.tar.gz
else 
...
echo "copy [rootfs]"
if [ x${ROOTFS_VERSION} = 'xubuntu18044' ]; then
	tar -zpxvf ../image/ubuntu18044.tar.gz -C mount_point0/ &> /dev/null
else
...
echo "copy rootfs ..."
if [ x${ROOTFS_VERSION} = 'xubuntu18044' ]; then
	cp -arxf ../image/ubuntu18044.tar.gz mount_point0/rootfs.tar.gz &> /dev/null
else
